...
public class Get_SHA512 {
	...
	protected static String randomSession9dec(int seed) {
		...
		String hash = Get_SHA512.get_SHA_512_SecurePassword(cur_time_string, Integer.toString(seed));

		int k = cur_time_string.charAt(cur_time_string.length() - 1) - '0';
		hash = Long.toString(Long.parseUnsignedLong(hash.substring(k, k + 10), 16));
	...
	protected static String randomIPhex(int seed) {
		...
		for (int i = 0; i < 4; i++) {
			ip[i] = Integer.toString(Integer.parseUnsignedInt(hash.substring(i * 2, i * 2 + 2), 16));
		}
